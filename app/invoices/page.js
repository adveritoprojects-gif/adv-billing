    ...
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <div className="relative">
          ...
          <TextInput value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search patient or invoice #" className="pl-8 w-56" />
        </div>
        {isAdvanced && (
            ...
            {["All", "Paid", "Partial", "Unpaid"].map((s) => (
              <button
                ...
                className={`rounded-full px-3 py-1.5 text-xs font-medium ${
                  filter === s ? "bg-tealDeep text-white" : "bg-surfaceAlt text-inkSoft"
                }`}
              >
                {s}
              </button>
      ...
      <div className="flex flex-col gap-2">
        {filtered.slice().reverse().map((inv) => (
          <div key={inv.id} className="rounded-lg bg-white overflow-hidden border border-border">
            <button
              ...
              <div className="px-4 pb-4 text-sm border-t border-border">
                ...
                {isAdvanced && (
                  <div className="mt-1 space-y-1 text-xs text-inkSoft">
                    {inv.discountAmt > 0 && <div className="flex justify-between"><span>Discount</span><span>−{fmt(inv.discountAmt)}</span></div>}
                    {inv.taxAmt > 0 && <div className="flex justify-between"><span>Tax</span><span>+{fmt(inv.taxAmt)}</span></div>}
                    {inv.insuranceAmt > 0 && <div className="flex justify-between"><span>Insurance</span><span>−{fmt(inv.insuranceAmt)}</span></div>}
                  </div>
                )}
                <div className="flex justify-between pt-2 mt-1 border-t border-border text-sm text-ink">
                  <span>Paid via {inv.mode}</span><span className="tabular-nums">{fmt(inv.paid)}</span>
                </div>
                {inv.due > 0.005 && (
                  <div className="flex justify-between text-sm text-rose"><span>Due</span><span className="tabular-nums">{fmt(inv.due)}</span></div>
                )}
              </div>
            )}
          </div>
        ))}
        {filtered.length === 0 && <p className="text-sm py-6 text-center text-inkSoft">No invoices match.</p>}
      </div>
    </div>
  );
}
